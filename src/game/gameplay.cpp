...
void invokePair(const ParallelExecutor* parallel,
                ...
                ParallelExecutor::Function second, void* secondContext) noexcept;
...
std::uint16_t impactLifetimeTicks(ImpactEffectType type) noexcept {
    ...
    case ImpactEffectType::BossDeath: return 31;
    }
    return 13;
}
...
float aiDangerAt(
    ...
    auto danger = 0.0F;
    constexpr float kEnemyQueryRadius = 100.0F + kMaximumEnemyRadius;
    enemyGrid.forEachCandidate(x, y, kEnemyQueryRadius, [&](std::size_t index) {
        const auto& enemy = enemies[index];
        if (!enemy.active || enemy.hp <= 0 || enemy.bornTicks > 0
            || enemy.spawnDelayTicks > 0) return;
        if (!aiHazardVisible(observer, enemy.x, enemy.y, index)) return;
    ...
    // 最速弾(124px/s)を42tick予測した移動量と最大危険半径を包含する。
    constexpr float kEnemyBulletQueryRadius =
        (124.0F / 60.0F) * 42.0F
        + kPlayerRadius + kMaximumEnemyBulletRadius + 22.0F;
    enemyBulletGrid.forEachCandidate(
        x, y, kEnemyBulletQueryRadius, [&](std::size_t index) {
        const auto& bullet = enemyBullets[index];
        if (!bullet.active || bullet.launchDelayTicks > 0) return;
        if (!aiHazardVisible(observer, bullet.x, bullet.y, index + 1000U)) return;
...
float aiMoveScore(
    ...
    auto score = rescuing ? 0.0F : aiDangerAt(
        x, y, enemies, enemyBullets, enemyGrid, enemyBulletGrid,
...
void scoreAiDirections(void* context) noexcept {
    const auto& job = *static_cast<AiDirectionJob*>(context);
    for (auto index = job.begin; index < job.end; ++index) {
        auto directionX = (*job.directions)[index][0];
        auto directionY = (*job.directions)[index][1];
        normalize(directionX, directionY);
        auto probeX = job.player->x;
        auto probeY = job.player->y;
        probeAiMovement(
            *job.map, job.player->x, job.player->y,
            directionX * job.lookahead, directionY * job.lookahead,
            probeX, probeY);
        const auto moved = std::sqrt(squaredDistance(
            job.player->x, job.player->y, probeX, probeY));
        if (moved < 0.05F) continue;
        directionX = probeX - job.player->x;
        directionY = probeY - job.player->y;
        normalize(directionX, directionY);
        ...
        result.score = aiMoveScore(
            ...
            *job.enemyGrid, *job.enemyBulletGrid,
...
float moveAiPlayerSmart(
    ...
    const auto currentDanger = rescuing
        ? 0.0F : aiDangerAt(
            player.x, player.y, enemies, enemyBullets, enemyGrid, enemyBulletGrid,
            useAttention ? &player : nullptr);
    ...
    if (player.aiDecisionTicks == 0 || (!useAttention && urgent)) {
        ...
        auto retained = false;
        std::array<AiDirectionResult, 18> results{};
        const auto directionCount =
            static_cast<std::size_t>(directionSteps + 2U);
        const auto split = (directionCount + 1U) / 2U;
        std::array<AiDirectionJob, 2> jobs{{
            {
                &player, &leader, &enemies, &enemyBullets,
                &enemyGrid, &enemyBulletGrid, &map,
        ...
        invokePair(
            parallel,
            scoreAiDirections, &jobs[0],
            scoreAiDirections, &jobs[1]);
        for (std::size_t index = 0; index < directionCount; ++index) {
            ...
            if (index == 0U) {
                retained = true;
                retainedScore = score;
                retainedDirectionX = result.directionX;
                retainedDirectionY = result.directionY;
            }
            if (score >= bestScore) continue;
            bestScore = score;
            bestDirectionX = result.directionX;
            bestDirectionY = result.directionY;
        }
...
void followAiPartner(PlayerState& player, std::size_t playerIndex,
    ...
    const auto currentDanger = aiDangerAt(
        player.x, player.y, enemies, enemyBullets,
        enemyGrid, enemyBulletGrid, &player);
            ...
            for (float radius = 4.0F; radius <= 32.0F && !found; radius += 4.0F) {
                for (std::uint8_t index = 0; index < 24U; ++index) {
                    ...
                    const auto score = std::sqrt(squaredDistance(
                        ...
                        + (rescuing ? 0.0F
                            : aiDangerAt(
                                x, y, enemies, enemyBullets,
                                enemyGrid, enemyBulletGrid, &player) * 0.03F)
                        + radius * 0.15F;
                    if (score >= bestScore) continue;
                    bestScore = score;
                    bestX = x;
...
void moveAttractPlayer(
    ...
    if (partner.hp <= 0) {
        (void)moveAiPlayerSmart(player, partner.x, partner.y, partner,
                                enemies, enemyBullets, enemyGrid, enemyBulletGrid,
                                map, true, false, parallel);
        return;
    }
...
EnemyState* nearestEnemy(std::array<EnemyState, kMaximumEnemies>& enemies,
    ...
    enemyGrid.forEachCandidate(x, y, range, [&](std::size_t enemyIndex) {
        auto& enemy = enemies[enemyIndex];
        if (!enemy.active || enemy.bornTicks > 0 || enemy.spawnDelayTicks > 0) return;
        const auto candidate = squaredDistance(x, y, enemy.x, enemy.y);
        if (candidate < nearestSquared
            || (candidate == nearestSquared && (nearest == nullptr || enemyIndex > nearestIndex))) {
            nearestSquared = candidate;
            nearest = &enemy;
...
EnemyState makeEnemyState(EnemyKind kind, float x, float y,
    ...
    case EnemyKind::Mage:
        ...
        break;
    case EnemyKind::Boss:
        enemy.radius = kMaximumEnemyRadius;
        enemy.hp = 900;
        enemy.xpValue = 0;
        enemy.speedPerTick = 24.0F / 60.0F;
    ...
    if (enemy.endlessScaled) {
        ...
        enemy.hp = static_cast<std::int16_t>(std::min(
            32767.0F, std::round(static_cast<float>(enemy.hp) * hpScale
...
void spawnThunder(PlayerState& player, std::uint8_t owner,
    ...
    for (std::uint8_t strikeIndex = 0; strikeIndex < stats.count; ++strikeIndex) {
        ...
        slot->ageTicks = 0;
        slot->active = true;
        enemyGrid.forEachCandidate(slot->x, slot->y, slot->radius + kMaximumEnemyRadius,
                                  [&](std::size_t enemyIndex) {
            auto& enemy = enemies[enemyIndex];
            if (!enemy.active) return;
            const auto range = slot->radius + enemy.radius;
            if (squaredDistance(slot->x, slot->y, enemy.x, enemy.y) > range * range) return;
...
void damageOrbs(PlayerState& player, std::uint8_t owner,
    ...
    for (std::uint8_t orbIndex = 0; orbIndex < stats.count; ++orbIndex) {
        ...
        enemyGrid.forEachCandidate(x, y, 4.0F + kMaximumEnemyRadius,
            ...
            if (squaredDistance(x, y, enemy.x, enemy.y) >= range * range) return;
            enemy.hp = static_cast<std::int16_t>(enemy.hp - stats.damage);
...
void updateFamiliars(PlayerState& player, std::uint8_t owner,
    ...
    for (std::uint8_t index = 0; index < kMaximumFamiliarsPerPlayer; ++index) {
        ...
        if (auto* target = nearestEnemy(enemies, enemyGrid, homeX, homeY, seekRange)) {
    ...
    for (auto& familiar : player.familiars) {
        if (!familiar.active) continue;
        auto* target = nearestEnemy(enemies, enemyGrid, familiar.x, familiar.y, fireRange);
...
void processBombs(std::array<PlayerState, pixel_twins::kControllerCount>& players,
    ...
    for (std::size_t playerIndex = 0; playerIndex < players.size(); ++playerIndex) {
        ...
        enemyGrid.forEachCandidate(player.x, player.y, kRadius + kMaximumEnemyRadius,
                                   [&](std::size_t enemyIndex) {
            auto& enemy = enemies[enemyIndex];
            if (!enemy.active || enemy.bornTicks > 0) return;
            const auto range = kRadius + enemy.radius;
            if (squaredDistance(player.x, player.y, enemy.x, enemy.y) > range * range) return;
    ...
    for (auto& bullet : bullets) {
        ...
        if (bullet.type == PlayerAttack::Light && bullet.ageTicks >= 10) {
            if (auto* target = nearestEnemy(
                    enemies, enemyGrid, bullet.x, bullet.y, kLightHomingRange)) {
        ...
        auto hitEnemyIndex = enemies.size();
        enemyGrid.forEachCandidate(
            bullet.x, bullet.y, bullet.radius + kMaximumEnemyRadius,
            [&](std::size_t enemyIndex) {
                auto& enemy = enemies[enemyIndex];
                if (!enemy.active || enemyIndex >= hitEnemyIndex) return;
                const auto hitRange = enemy.radius + bullet.radius;
                if (squaredDistance(bullet.x, bullet.y, enemy.x, enemy.y)
                    < hitRange * hitRange) {
                    hitEnemy = &enemy;
        ...
        if (hitEnemy != nullptr) {
            hitEnemy->hp = static_cast<std::int16_t>(hitEnemy->hp - bullet.damage);
            if (bullet.type == PlayerAttack::Ice) {
                hitEnemy->slowTicks = std::max<std::uint16_t>(hitEnemy->slowTicks, 102);
            }
            ...
            if (bullet.type == PlayerAttack::Fire) impactType = ImpactEffectType::Fire;
                ...
                impactType = ImpactEffectType::Familiar;
            }
...
void updateWindSlashes(std::array<WindSlashState, kMaximumWindSlashes>& slashes,
    ...
    for (auto& slash : slashes) {
        ...
        enemyGrid.forEachCandidate(
            owner.x, owner.y, slash.outerRadius + kMaximumEnemyRadius,
            [&](std::size_t enemyIndex) {
                auto& enemy = enemies[enemyIndex];
                if (!enemy.active || slash.hitCooldownTicks[enemyIndex] > 0) return;
                const auto distanceSquared =
                    squaredDistance(owner.x, owner.y, enemy.x, enemy.y);
                ...
                if (!hit) {
                    ...
                    const auto cutterRadius =
                        (slash.innerRadius + slash.outerRadius) * 0.5F;
                ...
                if (enemy.hp <= 0) {
                    killEnemy(enemy, slash.owner, xpGems, scores, killCounts);
                }
            });
    }
}
...
void beginPerkChoice(PlayerState& player, std::uint32_t& randomState,
                     bool endless) noexcept {
    if (player.choosingPerk || player.pendingPerkChoices == 0) return;
    rollPerks(player, randomState, endless);
...
bool spawnEndlessBossWave(
    ...
    const auto count = static_cast<std::uint8_t>(
        targetCount > living ? targetCount - living : 0U);
    ...
    const auto& focus = players[players[focusIndex].hp > 0 ? focusIndex : 1U - focusIndex];
    const auto phase = randomUnit(randomState) * kTau;
    const auto step = kTau / static_cast<float>(count);
    for (std::uint8_t index = 0; index < count; ++index) {
        auto placed = false;
        const auto preferredAngle = phase + static_cast<float>(index) * step;
        for (std::uint8_t attempt = 0; attempt < 36 && !placed; ++attempt) {
            const auto angle = preferredAngle + (randomUnit(randomState) - 0.5F) * 0.36F;
            ...
            auto overlapsNew = false;
            for (std::uint8_t other = 0; other < index; ++other) {
                overlapsNew = overlapsNew
                    || squaredDistance(x, y, positions[other][0], positions[other][1])
                        < 80.0F * 80.0F;
            }
    ...
    for (std::uint8_t index = 0; index < count; ++index) {
        ...
        *slot = makeEnemyState(EnemyKind::Boss, positions[index][0], positions[index][1],
                               elapsedTicks, randomState, true, balance);
    }
    return true;
}
...
void recycleDistantEnemiesForSpawn(
    std::array<EnemyState, kMaximumEnemies>& enemies,
    const std::array<PlayerState, pixel_twins::kControllerCount>& players,
    std::uint8_t requestedCount, std::size_t enemyLimit) noexcept {
    const auto activeCount = static_cast<std::size_t>(std::count_if(
        enemies.begin(), enemies.end(),
        [](const EnemyState& enemy) { return enemy.active; }));
    auto needed = activeCount + requestedCount > enemyLimit
        ? activeCount + requestedCount - enemyLimit
        : 0U;
    while (needed > 0U) {
...
void invokePair(const ParallelExecutor* parallel,
                ParallelExecutor::Function first, void* firstContext,
                ParallelExecutor::Function second, void* secondContext) noexcept {
    if (parallel != nullptr && parallel->invokePair != nullptr) {
        parallel->invokePair(
            parallel->context, first, firstContext, second, secondContext);
        return;
    }
    first(firstContext);
...
struct PlayerUpdateJob {
    PlayerState* player;
    CameraState* camera;
    const PlayerState* partner;
    const std::array<EnemyState, kMaximumEnemies>* enemies;
    const std::array<EnemyBulletState, kMaximumEnemyBullets>* enemyBullets;
    const std::array<XpGemState, kMaximumXpGems>* xpGems;
    const EnemySpatialGrid* enemyGrid;
    ...
    std::size_t index;
    std::uint32_t attractElapsedTicks;
    bool manual;
    bool attract;
};

void updatePlayerJob(void* context) noexcept {
    auto& job = *static_cast<PlayerUpdateJob*>(context);
    auto& player = *job.player;
    if (player.hp > 0) {
        if (job.manual) {
            movePlayer(player, *job.controller, *job.map);
        } else if (job.attract) {
            moveAttractPlayer(
    ...
    if (player.invulnerabilityTicks > 0) --player.invulnerabilityTicks;
    if (player.lightCooldownTicks > 0) --player.lightCooldownTicks;
    if (player.fireCooldownTicks > 0) --player.fireCooldownTicks;
    if (player.windCooldownTicks > 0) --player.windCooldownTicks;
    if (player.thunderCooldownTicks > 0) --player.thunderCooldownTicks;
    if (player.iceCooldownTicks > 0) --player.iceCooldownTicks;
    if (player.orbCooldownTicks > 0) --player.orbCooldownTicks;
    if (player.familiarCooldownTicks > 0) --player.familiarCooldownTicks;
    if (player.xpRecallAiCooldownTicks > 0) --player.xpRecallAiCooldownTicks;
    if (player.hp > 0) {
        player.orbAngle -=
            (8.1F + static_cast<float>(player.orbLevel) * 0.72F) / 60.0F;
        if (player.hp < player.maxHp) {
            player.hpRegenAccumulator += 0.225F / 60.0F;
            if (player.hpRegenAccumulator >= 1.0F) {
                ++player.hp;
                player.hpRegenAccumulator -= 1.0F;
            }
        } else {
            player.hpRegenAccumulator = 0.0F;
        }
    }
    updateCamera(*job.camera, player);
}
...
void rebuildEnemyGridJob(void* context) noexcept {
    auto& job = *static_cast<EnemyGridJob*>(context);
    job.grid->rebuild(*job.enemies);
}

struct EnemyBulletGridJob {
    EnemyBulletSpatialGrid* grid;
    const std::array<EnemyBulletState, kMaximumEnemyBullets>* bullets;
};

void rebuildEnemyBulletGridJob(void* context) noexcept {
    auto& job = *static_cast<EnemyBulletGridJob*>(context);
    job.grid->rebuild(*job.bullets);
}

struct EnemyBulletUpdateJob {
    std::array<EnemyBulletState, kMaximumEnemyBullets>* bullets;
    std::array<PlayerState, pixel_twins::kControllerCount>* players;
    const BalanceProfile* balance;
};

void updateEnemyBulletJob(void* context) noexcept {
    auto& job = *static_cast<EnemyBulletUpdateJob*>(context);
    updateEnemyBullets(*job.bullets, *job.players, *job.balance);
}

void updateThunderJob(void* context) noexcept {
    updateThunderStrikes(
        *static_cast<std::array<ThunderStrikeState, kMaximumThunderStrikes>*>(
            context));
}
...
void GameplayState::tick(const pixel_twins::Controllers& controllers,
    ...
    updatePlayerJob(&playerJobs[0]);
    updatePlayerJob(&playerJobs[1]);
    ...
    if (difficulty_ == Difficulty::Endless && elapsedTicks_ >= nextEndlessBossTicks_) {
        const auto target = static_cast<std::uint8_t>(
            std::min<std::uint8_t>(4U, endlessBossWave_ + 1U));
        if (spawnEndlessBossWave(enemies_, players_, map, target, elapsedTicks_,
                                 randomState_, balance)) {
            ++endlessBossWave_;
            endlessSpawnPauseTicks_ = 30U * 60U;
            bossSpawned_ = true;
            nextEndlessBossTicks_ = elapsedTicks_ + 2U * 60U * 60U;
        }
    }
    const auto spawningPaused = endlessSpawnPauseTicks_ > 0;
    if (spawningPaused) {
        --endlessSpawnPauseTicks_;
    } else {
    ...
    if (spawnCooldownTicks_ == 0 && !spawningPaused
        ...
        recycleDistantEnemiesForSpawn(enemies_, players_, requestedCount, enemyLimit);
        const auto availableCount = enemyCount() < enemyLimit
            ? enemyLimit - enemyCount() : 0U;
    ...
    invokePair(
        parallel,
        rebuildEnemyGridJob, &enemyGridJob,
        rebuildEnemyBulletGridJob, &enemyBulletGridJob);
    for (std::size_t index = 0; index < players_.size(); ++index) {
        auto& player = players_[index];
        if (player.hp > 0) {
        ...
        if (player.hp > 0 && player.thunderLevel > 0 && player.thunderCooldownTicks == 0) {
            spawnThunder(player, static_cast<std::uint8_t>(index), randomState_, thunderStrikes_,
                         enemies_, enemyGrid, xpGems_, scores_, killCounts_);
    ...
    EnemyBulletUpdateJob enemyBulletUpdateJob{
        &enemyBullets_, &players_, &balance};
    invokePair(
        parallel,
        updateThunderJob, &thunderStrikes_,
        updateEnemyBulletJob, &enemyBulletUpdateJob);
