...
int main() {
    ...
    assert(game.statistics().totalPlays == 1U);
    assert(game.statistics().normalPlays == 1U);
    assert(game.statistics().hardPlays == 0U);
    assert(game.rankingsDirty());
    assert(!game.setDifficulty(wizward::game::Difficulty::Hard));
    assert(game.difficulty() == wizward::game::Difficulty::Easy);
    ...
    wizward::game::PlayStatistics restoredStatistics{};
    restoredStatistics.totalPlays = 123U;
    restoredStatistics.normalPlays = 100U;
    restoredStatistics.hardPlays = 20U;
    restoredStatistics.endlessPlays = 3U;
    restoredStatistics.clears = 42U;
    restoredStatistics.totalPlayTicks = 987654U;
    restoredRankingGame.loadStatistics(restoredStatistics);
    restoredRankingGame.resetRankings();
    assert(restoredRankingGame.rankingCount() == 0U);
    assert(restoredRankingGame.endlessRankingCount() == 0U);
    assert(restoredRankingGame.statistics().totalPlays == 123U);
    assert(restoredRankingGame.rankingsDirty());
