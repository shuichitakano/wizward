...
DIP SW 1（GPIO26、ONでGNDへ接続）をONにすると、320×120画面の上部へデバッグ表示を
オーバーレイします。上段がcore 0、下段がcore 1で、各バーは高さ1ピクセルです。
左端の2ピクセルを余白とし、画面中央までを1/60秒（16,667µs）とします。超過分は右半分へ
伸び、約2/60秒で右端に達して飽和します。

- 赤：ゲーム描画とLED表示処理
- 緑：core 0の音声バッファ生成
- 青：ゲーム状態更新
- 白：USB処理、フレーム同期待ちなどのその他
- 黒：1/60秒枠内の未使用時間

音声生成はDMA割り込みで更新・描画へ割り込むため、累積音声生成時間を各区間から差し引き、
緑と青／赤へ二重計上しません。FPSは約0.5秒間の実フレーム数と経過時間から小数1桁で
算出します。core 0の次フレーム待ちとcore 1のPWM走査完了待ちは負荷へ含めません。
TinyUSBのホストタスクはフレーム先頭で1回実行し、音声割り込みは実行された時間を緑へ
含めます。DIP SWは毎フレーム読むため、再起動せず表示を切り替えられます。

デバッグ表示が有効なゲーム中は、PSボタンとの同時押しで次の操作ができます。

- PS＋上：操作中のプレイヤーをフルパワーアップ
- PS＋左：操作中のプレイヤーの次のパワーアップ選択をチャージ
- PS＋右：敵出現などの進行スケジュールを1分進める
